...
export default function CoinDetails() {
  const { back, DetailsDetails } = useContext(CoinContext);
  const marketCapChange =
    DetailsDetails?.market_data?.price_change_percentage_24h;
  ...
  if (high24hData) {
    ...
    return (
      <Stack className=" gap-[clamp(20px,5vw,60px)] pb-5">
        ...
        <Flex
          className="!mt-[clamp(80px,9vw,110px)] max-w-[1440px] m-auto "
          direction="column"
          ...
          <Flex direction="column" gap="clamp(20px,6vw,96px)">
            ...
            <Flex w="100%" direction="column" align="center" gap={17}>
              <Flex direction="column" gap={7} w="100%" align="center">
                ...
                <Text
                  className="text-center text-[#797979] lg:text-[14px] leading-7 items-center w-full"
                  maw={500}
                >
              ...
              >
                <Text className="text-[#212326] text-[clamp(20px,2vw,35px)] font-bold">
                  {`$${DetailsDetails?.market_data?.current_price[
                    "usd"
                  ].toLocaleString()}`}
                </Text>
                <Flex>
                  ...
                  >
                    ...
                    <span className="text-[clamp(14px,1vw,16px)] text-[#797979]">
                      (24hrs)
                    </span>
                  </Text>
                </Flex>
            ...
            >
              <Flex w="49%" direction="column" className="lg:w-full">
                ...
                <Text className="text-[#797979] leading-7 lg:text-[14px]">
                  {DetailsDetails?.description?.en.length > 0
                    ? DetailsDetails?.description?.en.slice(0, 1000)
                    : "No discription yet"}
                  .
                </Text>
              </Flex>
              ...
              >
                <Flex
                  direction="column"
                  gap={24}
                  className=" py-[clamp(15px,2vw,30px)]"
                >
                  <Title
                    className="px-[clamp(16px,4vw,60px)] text-[clamp(18px,1.5vw,25px)]"
                    c="#fff"
                  >
                    24hrs low and high
                  </Title>
                  ...
                  >
                    ...
                    <Flex direction="column" className="clg:text-[14px]">
                      ...
                      <Text c="#3EBA59">
                        $
                        {DetailsDetails?.market_data.high_24h[
                          "usd"
                        ].toLocaleString()}
                      </Text>
                    </Flex>
                    <Flex direction="column" className="clg:text-[14px]">
                      ...
                      <Text c="#3EBA59">
                        $
                        {DetailsDetails?.market_data.ath[
                          "usd"
                        ].toLocaleString()}
                      </Text>
                    </Flex>
                ...
                >
                  <Title
                    ...
                    c="#fff"
                  >
                    {DetailsDetails?.name} Market Information
                  </Title>
                  <Flex
                    gap={8}
                    justify="space-between"
                  ...
                  >
                    ...
                    <Flex direction="column" className="clg:text-[14px]">
                      ...
                      <Text c="#3EBA59">
                        $
                        {formatNumberInBillions(
                          Number(
                            DetailsDetails?.market_data?.total_volume["usd"]
                          )
                        )}
                      </Text>
                    </Flex>
                    <Flex direction="column" className="clg:text-[14px]">
                      ...
                      <Text c="#3EBA59">
                        $
                        {formatNumberInBillions(
                          Number(DetailsDetails?.market_data?.market_cap["usd"])
                        )}
                      </Text>
                    </Flex>
                  </Flex>
